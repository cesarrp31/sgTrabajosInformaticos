...
import org.legislaturachaco.sgTrabajosInformaticos.utilidades.logueo.ldap.ConexionLDAP;
...
public class LoginBean implements Serializable {
    ...
    public String login() {
        ...
        */
        ...
        //1º
        if(verificarUsuarioLocalBD(nombre, clave)){
            
        }else{//2
             if(verificarUsuarioDominio(nombre, clave)){
    ...
    private boolean verificarUsuarioLocalBD(String usuario, String clave){
        boolean resultado= false;
        return resultado;
    }
    
    private boolean verificarUsuarioDominio(String usuario, String clave){
        boolean resultado= false;
        ProcesoLogeoDominio pld= new ProcesoLogeoDominio();
        try{
            pld.verificarUsuarioDominio(usuario, clave);
            ...
            manejarErrores(une, "Error de autenticación");
        } catch (Exception ex) {
    ...
    public static void main(String[] args) {
        ...
        lb.setNombre("accesotrabinf1");
        lb.setClave("trabinfacceso2016");
        
        lb.login();
        
        System.out.println("Esta logeado?: "+lb.estaLogeado());
